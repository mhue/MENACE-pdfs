...
with open('output/symmetry_to_index_second.txt', 'w') as f:
    for s, idx in sorted(sym_to_index.items()):
        f.write(f'"{s}",{idx}\n')

    # Check for specific starting positions
    single_o_positions = ['o........', '.o.......', '..o......', '...o.....', '....o....']
    for pos in single_o_positions:
        if pos in sym_to_index:
            f.write(f"# Position {pos} is present with index {sym_to_index[pos]}\n")
        else:
            f.write(f"# Position {pos} is NOT present in the mapping.\n")
