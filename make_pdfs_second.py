...
from colors import COLOR_NAMES_SECOND
...
for i, boards in enumerate(positions):
    ...
    for j, board in enumerate(boards):
        latex += board.as_latex(index=index, palette=COLOR_NAMES_SECOND)
        latex += "\n"
        if (j + 1) % 5 == 0:
            latex += "\n\\noindent"
        index += 1
...
for i, boards in enumerate(positions):
    ...
    for j, board in enumerate(boards):
        ...
        latex += board.as_latex(index=index, best_moves=best_moves, palette=COLOR_NAMES_SECOND)
        latex += "\n"
        if (j + 1) % 5 == 0:
            latex += "\n\\noindent"
        index += 1
