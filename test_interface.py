...
def test_known_positions():
    """Test some known positions to verify the interface will work correctly."""
    
    # Load the generated JavaScript data by importing the Python data directly
    # This is more reliable than parsing the JavaScript
    from generate_position_data import generate_first_player_mappings, generate_second_player_mappings
    ...
    if empty_board in first_player_mappings:
        print(f"✓ Empty board (machine first): {empty_board} -> Box {first_player_mappings[empty_board]}")
    else:
        print(f"✗ Empty board not found in first player mappings")
    
    # Test 2: Single O move (machine plays second)
    single_o = '........o'
    if single_o in second_player_mappings:
        print(f"✓ Single O move: {single_o} -> Box {second_player_mappings[single_o]}")
    else:
        print(f"✗ Single O move not found in second player mappings")
